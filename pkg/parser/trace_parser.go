...
type TraceParser interface {
	ParseFromPath(path string) (*feedback.SimplifiedJaegerTrace, error)
}
...
func (p *JaegerTraceParser) ParseFromPath(filePath string) ([]*feedback.SimplifiedJaegerTrace, error) {
    ...
    var result struct {
        Spans []*feedback.SimplifiedJaegerTrace `json:"spans"`
    }
