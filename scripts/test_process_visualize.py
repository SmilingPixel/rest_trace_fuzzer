...
- Parses edge coverage and status code count.
- Generates two SVG graphs:
  1. Edge Coverage vs. Test Process
  2. Covered Status Code Count vs. Test Process
...
def extract_test_scenarios(logs):
    pattern = re.compile(r'Finish execute current test scenario .*?UUID: ([a-f0-9\-]+).*?Edge coverage: ([0-9\.]+).*?covered status code count: (\d+)', re.IGNORECASE)
    test_data = []
    ...
    for log in logs:
        ...
        if match:
            uuid = match.group(1)
            edge_coverage = float(match.group(2))
            status_code_count = int(match.group(3))
            test_data.append((uuid, edge_coverage, status_code_count))
...
def plot_graphs(test_data, output_prefix="test_analysis"):
    ...
    edge_coverages = [data[1] for data in test_data]
    status_code_counts = [data[2] for data in test_data]
